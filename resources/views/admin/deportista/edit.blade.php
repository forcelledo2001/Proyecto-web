...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-10">
            <h2 class="text-black">Editar deportista</h2>
            <hr class="text-black mb-4" />
            <form action="{{ route('admin.deportista.update', $deportista) }}" method="POST" class="form">
                @csrf  @method('PUT')
                <div>
                    <label for="nombre" class="my-2">Nombre:</label>
                    <input type="text" class="form-control @error('nombre')
                        invalid
                    @enderror" name="nombre" placeholder="Nombre" value="{{ old('nombre', $deportista->nombre) }}">
                    @error('nombre')
                ...
                <div>
                    ...
                    <input type="number" class="form-control @error('edad')
                        invalid
                    @enderror" name="edad" placeholder="Correo" value="{{ old('edad', $deportista->edad) }}">
                    @error('edad')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                <div>
                    ...
                    <input type="number" min="0" class="form-control @error('cantidad_goles')
                        invalid
                    @enderror" name="cantidad_goles" placeholder="Cantidad goles" value="{{ old('cantidad_goles', $deportista->cantidad_goles) }}">
                ...
                <div>
                    ...
                    @enderror">
                        ...
                        @forelse ($equipos as $equipo)
                            <option value="{{ $equipo->id }}" @selected($deportista->equipo_id == $equipo->id)>{{ $equipo->nombre }}</option>
                        @empty
